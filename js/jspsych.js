...
jsPsych.pluginAPI = (function() {
  ...
  module.autoPreload = function(timeline, callback, images, audio, progress_bar) {
    ...
    var total_n = images.flat(1).length + audio.flat(1).length;
    console.log(total_n);
    var loaded = 0;

    if(progress_bar){
      var pb_html =  "<img src='img/star.png' style='position: absolute; height: 2vh; left: 93%; top: 33%;'>" +
                ...
                "<img id='jspsych-loading-img' src='img/small_rocket.png' style = 'position: absolute; left: -20%; top: 60%; height: 10%'>" +
                "<div style='font-family: Montserrat; font-size: 4vh; line-height: 1; font-weight: 600; text-align: center; width: 90%; color: white; position: fixed; top: 100%; left: 50%; transform: translate(-50%, -120%);'>THE TASK IS LOADING - PLEASE WAIT</div>";
    ...
    function update_loading_progress_bar(){
      ...
      if(progress_bar){
        ...
        jsPsych.getDisplayElement().querySelector('#jspsych-loading-img').style.marginLeft = percent_loaded+"%";
      }
    }
